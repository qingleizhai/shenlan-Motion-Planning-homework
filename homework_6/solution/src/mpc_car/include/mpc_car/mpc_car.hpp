...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,
    ...
    gd_(0) = v * phi * sin(phi) * dt_;
    gd_(1) = -v *phi * cos(phi) * dt_;
    gd_(2) = -v * delta / (ll_ * pow(cos(delta),2)) * dt_;
    gd_(3) = 0;
 ...
 public:
  MpcCar(ros::NodeHandle& nh) : nh_(nh) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      lu_.coeffRef(i * 3 + 1, 0) = -ddelta_max_;
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i == 0) {
        ...
        // ...
        Pgd_ = gd_ + Ad_ * gd_;
        gg.block(n * i, 0, n, 1) = Pgd_;
        BB.block(n * i, i * m, n, m) = Bd_;
        BB.block(n * i, 0, n, m * i) = Ad_ * BB.block((i - 1) * n, 0, n, m * i);
